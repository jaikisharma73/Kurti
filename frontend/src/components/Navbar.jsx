  ...
  return (
    ...
    >
      <div className='flex items-center relative'>
        <img onClick={() => setVisible(!visible)} src={assets.menu_icon.src || assets.menu_icon} className='w-6 cursor-pointer invert hover:opacity-80 transition-opacity relative z-[101]' alt="Menu" />
        ...
        <div className={`absolute left-10 top-1/2 -translate-y-1/2 flex items-center justify-between w-[240px] sm:w-[320px] md:w-[400px] text-[10px] sm:text-xs md:text-sm font-serif tracking-widest text-gray-200 mix-blend-difference px-2 transition-all duration-500 z-[100] ${visible ? 'opacity-100 translate-x-0 pointer-events-auto' : 'opacity-0 -translate-x-8 pointer-events-none'}`}>
          <Link onClick={() => setVisible(false)} className='hover:text-[#c8a06e] transition-colors' href='/'>HOME</Link>
          <Link onClick={() => setVisible(false)} className='hover:text-[#c8a06e] transition-colors' href='/collection'>COLLECTION</Link>
          <Link onClick={() => setVisible(false)} className='hover:text-[#c8a06e] transition-colors' href='/about'>ABOUT</Link>
          <Link onClick={() => setVisible(false)} className='hover:text-[#c8a06e] transition-colors' href='/contact'>CONTACT</Link>
        </div>
      </div>
      <div className='flex items-center gap-6 ml-auto'>
        <img onClick={() => { setShowSearch(true); navigate('/collection') }} src={assets.search_icon.src || assets.search_icon} className='w-5 cursor-pointer invert' alt="" />
        <div className='group relative'>
          <img onClick={() => token ? null : navigate('/login')} className='w-5 cursor-pointer invert' src={assets.profile_icon.src || assets.profile_icon} alt="" />
          {token && (
        ...
        <Link href='/cart' className='relative group/cart'>
          <img src={assets.cart_icon.src || assets.cart_icon} className='w-5 min-w-5 invert' alt="" />
